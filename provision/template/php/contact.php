<?php
/**
 * Client demo site contact handler for {{BUSINESS_NAME}}
 * Subdomain: {{SUBDOMAIN}}
 *
...
header('Access-Control-Allow-Origin: https://{{SUBDOMAIN}}');
...
if ($error) {
    ...
    $subject = "Lead from {{SUBDOMAIN}}: " . ($data['name'] ?? 'Unknown');
    $body    = "Source: {{SUBDOMAIN}}\n\n";
    foreach ($data as $key => $value) {
        $body .= ucfirst($key) . ": " . $value . "\n";
    }
    $headers = "From: {{BUSINESS_NAME}} <noreply@drape.digital>\r\n";
